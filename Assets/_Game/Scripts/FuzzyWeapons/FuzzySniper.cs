...
public class FuzzySniper : FuzzyLogic
{
    //ammo curves
    ...
    public override float[] FuzziRulesOutput(float[] distanceValues, float[] ammoValues)
    {
        ...
        // Rule 1 IF Target_Far AND Ammo_Loads THEN VeryDesirable
        desirabilityValues[2] = Mathf.Max(desirabilityValues[2], Mathf.Min(distanceValues[2], ammoValues[2]));
        // Rule 2 IF Target_Far AND Ammo_Okay THEN VeryDesirable
        desirabilityValues[2] = Mathf.Max(desirabilityValues[2], Mathf.Min(distanceValues[2], ammoValues[1]));
        // Rule 3 IF Target_Far AND Ammo_Low THEN VeryDesirable
        desirabilityValues[2] = Mathf.Max(desirabilityValues[2], Mathf.Min(distanceValues[2], ammoValues[0]));
        // Rule 4 IF Target_Medium AND Ammo_Loads THEN VeryDesirable
        desirabilityValues[2] = Mathf.Max(desirabilityValues[2], Mathf.Min(distanceValues[1], ammoValues[2]));
        // Rule 5 IF Target_Medium AND Ammo_Okay THEN Desirable
        desirabilityValues[1] = Mathf.Max(desirabilityValues[1], Mathf.Min(distanceValues[1], ammoValues[1]));
        ...
        desirabilityValues[0] = Mathf.Max(desirabilityValues[0], Mathf.Min(distanceValues[0], ammoValues[2]));
    ...
    public float FuzzySniperSystem()
    {
    ...
    private void Update()
    {
        float desirability = FuzzySniperSystem();
        Debug.Log("Desirability Sniper:  " + desirability);
    }
}
