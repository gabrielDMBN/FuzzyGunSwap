...
//     High = 2
// }
public class FuzzyPistol : FuzzyLogic
{
    //ammo curves
    ...
    public override float[] FuzziRulesOutput(float[] distanceValues, float[] ammoValues)
    {
        ...
        desirabilityValues[1] = Mathf.Max(desirabilityValues[1], Mathf.Min(distanceValues[2], ammoValues[2]));
        ...
        desirabilityValues[2] = Mathf.Max(desirabilityValues[2], Mathf.Min(distanceValues[1], ammoValues[1]));
        // Rule 6 IF Target_Medium AND Ammo_Low THEN Desirable
        desirabilityValues[1] = Mathf.Max(desirabilityValues[1], Mathf.Min(distanceValues[1], ammoValues[0]));
